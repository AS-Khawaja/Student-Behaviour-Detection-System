...
# ---------- history helpers (insert after imports / seed setup) ----------
import json
from datetime import datetime
import numpy as np

def _to_plain(o):
    """Convert numpy / datetime to JSON-safe plain Python types."""
...
def init_history(save_path="history.json", extra_info=None):
    """
    Create a fresh history dict, save an initial file and return the dict.
    """
    history = {
...
def update_history(history,
                   ...
                   autosave=True):
    """
    Append metrics for one epoch. Autosaves to history['save_path'] by default.
    """
    history["epochs_recorded"] = history.get("epochs_recorded", 0) + 1

    def _append(key, val):
        if key not in history:
            history[key] = []
        history[key].append(_to_plain(val))

    _append("loss", train_loss)
    _append("acc", train_acc)
    _append("val_loss", val_loss)
    _append("val_acc", val_acc)
    _append("val_prec", val_prec)
    _append("val_rec", val_rec)
    _append("val_f1", val_f1)

    ts = datetime.utcnow().isoformat() + "Z"
    history.setdefault("epoch_ts", []).append(ts)
    if epoch is not None:
        history.setdefault("epoch_idx", []).append(_to_plain(epoch))

    if extra_updates:
        if "extra" not in history:
            history["extra"] = {}
        for k, v in extra_updates.items():
            history["extra"].setdefault(k, []).append(_to_plain(v))

    if autosave:
        save_path = history.get("save_path", "history.json")
        try:
            with open(save_path, "w") as f:
                json.dump(history, f, indent=2)
        except Exception as e:
            print(f"[history] Warning: failed to save history to {save_path}: {e}")

    return history

def save_history(history, path=None):
    path = path or history.get("save_path", "history.json")
    try:
        with open(path, "w") as f:
            json.dump(history, f, indent=2)
    except Exception as e:
        print(f"[history] Warning: failed to save history to {path}: {e}")
...
def main():
    ...
    # 9) training loop
    best_val_f1 = 0.0
    history = init_history(os.path.join(out_dir, "history.json"))
