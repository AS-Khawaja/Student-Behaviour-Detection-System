...
# --------------------------------
# 4) Hybrid PyTorch model (EfficientNet-B0 backbone)
# --------------------------------
class HybridNet(nn.Module):
    """
    Hybrid network using EfficientNet-B0 as image backbone and a small MLP for metadata.
    Image features from EfficientNet are pooled and concatenated with metadata MLP output.
    """
    def __init__(self, metadata_dim, num_classes, pretrained=True, dropout=0.3):
        ...
        # Use feature extractor (all layers except classifier) + adaptive pooling to get fixed-size vector
        self.backbone = nn.Sequential(
            effnet.features,
            nn.AdaptiveAvgPool2d((1,1))
        )

        # backbone feature dim usually available at effnet.classifier[1].in_features
        try:
            backbone_feat_dim = effnet.classifier[1].in_features
        except Exception:
            backbone_feat_dim = 1280  # typical for B0
        ...
        # classifier combining image + metadata
